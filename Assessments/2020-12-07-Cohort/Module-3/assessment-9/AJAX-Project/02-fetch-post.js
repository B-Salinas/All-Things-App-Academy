...
async function addPerson() {
  let body = JSON.stringify({ firstName: 'Maul', lastName: 'Kimmerman' });
  let response = await fetch('http://example.com/api/people', {
    method: 'POST', 
    body
  });
  let jsond = await response.json();
  let { people } = jsond;
  let pplList = document.getElementById('people-list-02');

  for (let ppl of people) { // for/in is for objects BUT for/of is for arrays
    let liEl = document.createElement('li');
    liEl.innerHTML = `${ppl.lastName}, ${ppl.firstName}`;
    pplList.append(liEl);
  }
}
