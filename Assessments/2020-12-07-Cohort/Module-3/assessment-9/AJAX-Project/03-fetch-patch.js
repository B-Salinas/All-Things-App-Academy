...
async function updatePerson() {
  let body = JSON.stringify({ lastName: 'Ball' });
  let response = await fetch('http://example.com/api/people/3', {
    method: 'PATCH',
    body
  });
  let jsond = await response.json();
  let person = document.getElementById('person-3');
  person.innerHTML = `${jsond.lastName}, ${jsond.firstName}`;
}
